...
const createTeam = () => async (owner) => teamsModel
    .create({
    name: `${Date.now()}_test`,
...
const addTeamUserToTeam = (opt) => async (id, user, teamRoleName) => teamsModel
    .findOneAndUpdate({ _id: id }, { $push: { userIds: await createTeamUser(user._id, opt.schoolId, teamRoleName) } }, { new: true })
    .lean()
    .exec();
const getTeamById = (id) => teamsModel.findById(id).lean().exec();
const removeOneTeam = (id) => teamsModel.findOneAndRemove({ _id: id }).lean().exec();
const removeManyTeams = (ids) => teamsModel
