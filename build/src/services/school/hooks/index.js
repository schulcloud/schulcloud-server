...
const hasEditPermissions = async (context) => {
    try {
        ...
        for (const key of Object.keys(context.data)) {
            if ((user.permissions.includes('SCHOOL_CHAT_MANAGE') && updatesChat(key, context.data)) ||
...
const validateCounty = async (context) => {
    if (context && context.data && context.data.county) {
        ...
        if (!currentSchool.federalState.counties.length ||
...
exports.after = {
    all: [
        ...
        iff(populateInQuery, keepInArray('systems', [
            '_id',
            'type',
            'alias',
            'ldapConfig.active',
