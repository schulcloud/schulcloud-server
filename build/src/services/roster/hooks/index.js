...
module.exports = {
    tokenIsActive: (context) => context.app
    ...
    injectOriginToolIds: (context) => {
        if (!context.params.tokenInfo)
            throw new Error('Token info is missing in params'); // first call isTokenActive
    ...
    groupContainsUser: (context) => {
        ...
        if (users.find((user) => user.user_id ===
