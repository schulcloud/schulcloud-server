...
const { copyPropertyNameIfIncludedInValuesFromSourceToTarget, isValidNotFoundResponse, isValidFoundResponse, } = require('./logic/utils');
const server = require('./logic/server');
const { ROLES, PERMISSIONS, SCOPE_NAMES, RESPONSE_STATUS, STATES, CREATE_OPTION_TOGGLES, } = require('./logic/constants');
...
async function getScopeInfo(app, user, scopeName, scopeId) {
    ...
    if (permissionScopeName === 'teams')
        logoutURL += '?activeTab=events';
    if (permissionScopeName === 'courses')
        logoutURL += '?activeTab=tools';
    return {
        scopeTitle,
...
function getHighestVideoconferencePermission(permissions) {
    if (permissions.includes(PERMISSIONS.START_MEETING))
        return PERMISSIONS.START_MEETING;
    if (permissions.includes(PERMISSIONS.JOIN_MEETING))
        return PERMISSIONS.JOIN_MEETING;
    return null;
}
...
*/
function getSettings(userID, userPermissions, { options: { moderatorMustApproveJoinRequests = false, everybodyJoinsAsModerator = false, everyAttendeJoinsMuted = false, }, }, logoutURL = undefined) {
...
class GetVideoconferenceService {
    ...
    async get(scopeId, params) {
        ...
        if (isValidNotFoundResponse(meetingInfo)) {
            ...
            return createResponse(RESPONSE_STATUS.SUCCESS, wasRunning ? STATES.FINISHED : STATES.NOT_STARTED, userPermissionsInScope, hasStartPermission && hasOptions ? videoconferenceMetadata.options : {});
        }
        if (isValidFoundResponse(meetingInfo)) {
            return createResponse(RESPONSE_STATUS.SUCCESS, STATES.RUNNING, userPermissionsInScope, hasStartPermission && hasOptions ? videoconferenceMetadata.options : {});
        }
...
class CreateVideoconferenceService {
    ...
    async create(data = {}, params) {
        ...
        const { app } = this;
        const authenticatedUser = await getUser({ params, app });
        const { scopeTitle, userPermissionsInScope, logoutURL } = await getScopeInfo(app, authenticatedUser, scopeName, scopeId);
        // CHECK PERMISSIONS //////////////////////////////////////////////////////
        await throwOnFeaturesDisabled(authenticatedUser);
        ...
        try {
            ...
            if (hasStartPermission) {
                ...
                const { role, settings } = getSettings(authenticatedUser.id, userPermissionsInScope, videoconferenceMetadata, logoutURL);
            ...
            return createResponse(RESPONSE_STATUS.SUCCESS, STATES.RUNNING, userPermissionsInScope, hasStartPermission ? videoconferenceMetadata.options : {}, joinUrl);
        }
        catch (error) {
