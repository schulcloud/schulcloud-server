...
module.exports = {
    ...
    copyPropertyNameIfIncludedInValuesFromSourceToTarget: ({ source, propertyName, values, target, sourcePropertyNames = null, }) => {
    ...
    isValidNotFoundResponse: (response) => response && Array.isArray(response.messageKey) && response.messageKey.includes(MESSAGE_KEYS.NOT_FOUND),
    isValidFoundResponse: (response) => response && Array.isArray(response.returncode) && response.returncode.includes(RESPONSE_STATUS.SUCCESS),
};
