...
const validateCredentials = async (hook) => {
    ...
    if (!username)
        throw new BadRequest('no username specified');
    if (!password)
        throw new BadRequest('no password specified');
    if (!systemId)
        return hook;
    const { app } = hook;
...
const validatePassword = (hook) => {
    ...
    return Promise.all([
        ...
        if (password && !passwordVerification) {
            throw new Forbidden(`Du darfst das Passwort dieses Nutzers nicht ändern oder die
...
const checkExistence = (hook) => {
    ...
    return accountService.find({ query: { userId } }).then((result) => {
        ...
        const filtered = result.filter((a) => a.systemId === systemId);
        if (filtered.length > 0)
            return Promise.reject(new BadRequest('Der Account existiert bereits!'));
        return Promise.resolve(hook);
    });
};
