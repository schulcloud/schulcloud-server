...
class RocketChatUser {
    ...
    createRocketChatAccount(userId) {
        ...
        return this.app
            ...
            const createdUser = await request(getRequestOptions('/api/v1/users.create', body, true)).catch(async (err) => this.handleEmailInUse(err, email, password));
    ...
    async getOrCreateRocketChatAccount(userId) {
        try {
            ...
            if (!((scUser.schoolId.features || []).includes(SCHOOL_FEATURES.ROCKET_CHAT) || scUser.schoolId.purpose === 'expert')) {
...
//# sourceMappingURL=rocketChatUser.js.map