...
class RocketChatChannel {
    ...
    createChannel(teamId, name) {
        ...
        return this.app
            ...
            .then((team) => {
            currentTeam = team;
            const userNamePromises = currentTeam.userIds.map((user) => this.app
    ...
    async ensureRcChannel(channel, userId, teamId) {
        ...
        const rcChannelMembers = await request(getRequestOptions(`/api/v1/groups.members?roomName=${channel.channelName}`, {}, true, {}, 'GET')).catch((err) => this.handleChannelMissingRcSide(err, channel, teamId));
    ...
    async synchronizeModerators(teamId) {
        try {
            ...
            const channel = await channelModel.findOne({ teamId });
            if (channel) {
                const rcResponse = await request(getRequestOptions(`/api/v1/groups.moderators?roomName=${channel.channelName}`, {}, true, undefined, 'GET'));
                ...
                moderatorsToAdd.forEach((x) => request(getRequestOptions('/api/v1/groups.addModerator', { roomName: channel.channelName, userId: x }, true)));
                moderatorsToRemove.forEach((x) => request(getRequestOptions('/api/v1/groups.removeModerator', { roomName: channel.channelName, userId: x }, true)));
            }
            return Promise.resolve();
        }
        catch (err) {
...
//# sourceMappingURL=rocketChatChannel.js.map