...
const getEntryByActivationCode = async (ref, requestingId, activationCode) => {
    if (!requestingId)
        throw new Forbidden(customErrorMessages.NOT_AUTHORIZED);
    if (!activationCode)
        throw SyntaxError('activationCode required!');
...
const validEntry = async (entry) => {
    ...
    if (Date.parse(entry.updatedAt) + 1000 * Configuration.get('ACTIVATION_LINK_PERIOD_OF_VALIDITY_SECONDS') <
        Date.now()) {
...
const sendMail = async (ref, mail, entry) => {
    if (!mail || !mail.receiver || !mail.subject || !mail.content || !entry)
        throw SyntaxError('missing parameters');
    try {
