...
class LDAPSchoolSyncer extends Syncer {
    ...
    updateModifyTimestampMaximum(date) {
        if (date && this.stats.modifyTimestamp < date)
            this.stats.modifyTimestamp = date;
    }
    ...
    checkForUserChangesAndUpdate(idmUser, user) {
        ...
        return accountModel
            .update({ userId: user._id, systemId: this.system._id }, {
            username: `${this.school.ldapSchoolIdentifier}/${idmUser.ldapUID}`.toLowerCase(),
            userId: user._id,
            systemId: this.system._id,
            activated: true,
...
//# sourceMappingURL=LDAPSchoolSyncer.js.map