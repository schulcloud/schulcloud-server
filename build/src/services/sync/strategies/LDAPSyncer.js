...
class LDAPSyncer extends Syncer {
    ...
    async steps() {
        ...
        const nextSchoolSync = async (school) => {
            try {
                ...
                this.stats.errors.push(`LDAP sync failed for school "${school.name}" (${school._id}) with error "${err.message}".`);
            }
        };
    ...
    createSchoolsFromLdapData(data) {
        ...
        let newSchools = 0;
        let updates = 0;
        return Promise.all(data.map((ldapSchool) => this.app
            .service('schools')
            .find({
            query: {
...
//# sourceMappingURL=LDAPSyncer.js.map