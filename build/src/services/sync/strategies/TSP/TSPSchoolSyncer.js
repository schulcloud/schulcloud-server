...
const { TspApi, config: TSP_CONFIG, ENTITY_SOURCE, SOURCE_ID_ATTRIBUTE, createUserAndAccount, shortenedRegistrationProcess, } = require('./TSP');
...
class TSPSchoolSyncer extends mix(Syncer).with(ClassImporter) {
    ...
    async steps() {
        ...
        if (schools.length > 0) {
            ...
            [teacherMap, studentMap, classMap] = await Promise.all(['teachers', 'students', 'classes'].map((type) => this.fetch(type).then(this.createSchoolMap)));
            this.logInfo('Done.');
        }
    ...
    async createTeacher(tspTeacher, school, systemId) {
        try {
            ...
            const teacher = await createUserAndAccount(this.app, {
                namePrefix: tspTeacher.lehrerTitel,
                firstName: tspTeacher.lehrerVorname,
                lastName: tspTeacher.lehrerNachname,
                schoolId: school._id,
    ...
    async createStudent(tspStudent, school, systemId) {
        try {
            ...
            const student = await createUserAndAccount(this.app, {
                firstName: tspStudent.schuelerVorname,
                lastName: tspStudent.schuelerNachname,
                schoolId: school._id,
                source: ENTITY_SOURCE,
...
//# sourceMappingURL=TSPSchoolSyncer.js.map