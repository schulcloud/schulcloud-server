...
const checkTeamPermission = async ({ user, file, permission }) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const { role: creatorRole } = file.owner.userIds.find((_) => equalIds(_.userId, file.creator || file.permissions[0].refId));
...
const checkPermissions = (permission) => async (user, file) => {
    ...
    const { permissions, refOwnerModel, owner: { _id: owner }, } = fileObject;
...
//# sourceMappingURL=filePermissionHelper.js.map