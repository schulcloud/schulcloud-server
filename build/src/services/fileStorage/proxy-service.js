...
const { canWrite, canRead, canCreate, canDelete, returnFileType, generateFlatFileName, copyFile, createCorrectStrategy, createDefaultPermissions, createPermission, } = require('./utils');
...
const { FILE_PREVIEW_SERVICE_URI, FILE_PREVIEW_CALLBACK_URI, ENABLE_THUMBNAIL_GENERATION, FILE_SECURITY_CHECK_MAX_FILE_SIZE, SECURITY_CHECK_SERVICE_PATH, } = require('../../../config/globals');
const sanitizeObj = (obj) => {
    Object.keys(obj).forEach((key) => obj[key] === undefined && delete obj[key]);
    return obj;
};
const prepareThumbnailGeneration = (file, strategy, userId, { name: dataName }, { storageFileName, name: propName }) => ENABLE_THUMBNAIL_GENERATION
    ? Promise.all([
        ...
        strategy.generateSignedUrl({
            ...
            fileType: returnFileType(dataName || propName),
        }),
    ]).then(([downloadUrl, signedS3Url]) => rp
        .post({
...
const prepareSecurityCheck = (file, userId, strategy) => {
    if (Configuration.get('ENABLE_FILE_SECURITY_CHECK') === true) {
        if (file.size > FILE_SECURITY_CHECK_MAX_FILE_SIZE) {
            return FileModel.updateOne({ _id: file._id }, {
                $set: {
        ...
        return strategy
            ...
            const params = {
                ...
                body: {
                    ...
                    callback_uri: url.resolve(Configuration.get('API_HOST'), `${SECURITY_CHECK_SERVICE_PATH}${file.securityCheck.requestToken}`),
                },
                json: true,
            };
...
const fileStorageService = {
    ...
    find({ query, payload }) {
        ...
        return parentPromise
            .then(() => FileModel.find({ owner, parent: parent || { $exists: false } })
            .lean()
            .exec())
            .then((files) => Promise.all(files.map((f) => canRead(userId, f)
            .then(() => f)
            .catch(() => undefined))))
...
const signedUrlService = {
    ...
    async patch(id, data, params) {
        ...
        // deprecated: author check via file.permissions[0].refId is deprecated and will be removed in the next release
        const creatorId = fileObject.creator || fileObject.permissions[0].refPermModel !== 'user'
            ? userId
    ...
    async find({ query, payload }) {
        ...
        const actionMap = {
            user: () => {
                ...
                return Promise.all(userPermission.map(({ refId }) => userModel.findOne({ _id: refId }).exec())).then((result) => {
