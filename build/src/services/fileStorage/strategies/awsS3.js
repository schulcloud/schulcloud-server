...
const createAWSObject = async (schoolId) => {
    ...
    if (Configuration.get('FEATURE_MULTIPLE_S3_PROVIDERS_ENABLED') === true) {
        ...
        school.storageProvider.secretAccessKey = CryptoJS.AES.decrypt(school.storageProvider.secretAccessKey, S3_KEY).toString(CryptoJS.enc.Utf8);
        return {
...
const getFileMetadata = (storageContext, awsObjects, bucketName, s3) => {
    ...
    const getPath = (path) => {
        ...
        if (pathComponents[0] === '')
            pathComponents = pathComponents.slice(1); // omit leading slash
    ...
    return Promise.all(awsObjects.map((object) => headObject({ Bucket: bucketName, Key: object.Key }).then((res) => ({
        key: object.Key,
...
class AWSS3Strategy extends AbstractFileStorageStrategy {
    async create(schoolId) {
        ...
        return new Promise((resolve, reject) => promisify(awsObject.s3.createBucket.bind(awsObject.s3), awsObject.s3)({ Bucket: awsObject.bucket })
            .then((res) => {
    ...
    getFiles(userId, path) {
        ...
        return filePermissionHelper
            ...
            return createAWSObject(result.schoolId).then((awsObject) => {
                ...
                return promisify(awsObject.s3.listObjectsV2.bind(awsObject.s3), awsObject.s3)(params).then((res) => Promise.resolve(getFileMetadata(path, res.Contents, awsObject.bucket, awsObject.s3)));
            });
        });
    }
    copyFile(userId, oldPath, newPath, externalSchoolId) {
        ...
        return UserModel.userModel
            ...
            return createAWSObject(result.schoolId).then((awsObject) => {
                ...
                const params = {
                    Bucket: awsObject.bucket,
                    CopySource: `/${sourceBucket}/${encodeURIComponent(oldPath)}`,
                    Key: newPath,
                };
    ...
    generateSignedUrl({ userId, flatFileName, fileType, header }) {
        if (!userId || !flatFileName || !fileType) {
            return Promise.reject(new BadRequest('Missing parameters by generateSignedUrl.', { userId, flatFileName, fileType }));
        }
        return UserModel.userModel
            .findById(userId)
            .exec()
            ...
            return createAWSObject(result.schoolId).then((awsObject) => this.createIfNotExists(awsObject).then((safeAwsObject) => {
                const params = {
    ...
    deleteAllInDirectory(awsObject, params) {
        ...
        return promisify(awsObject.s3.listObjectsV2.bind(awsObject.s3), awsObject.s3)(params)
            ...
            if (deletionData.Deleted.length === 1000)
                return this.deleteAllInDirectory(awsObject, params);
