...
const validateRequest = (data, targetUser) => {
    ...
    if (!((data.parent_privacyConsent && data.parent_termsOfUseConsent) || (data.privacyConsent && data.termsOfUseConsent))) {
    ...
    if (!targetUser.importHash)
        return Promise.reject(new BadRequest('this user is not viable for registration'));
...
//# sourceMappingURL=SkipRegistration.js.map