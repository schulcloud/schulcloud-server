...
const getCurrentYear = (ref, schoolId) => ref.app
    .service('schools')
    .get(schoolId, {
...
class AdminUsers {
        ...
        try {
            ...
            if (_id) {
                ...
                if (typeof query._id === 'object')
                    query._id = Object.values(query._id);
            }
            if (clientQuery.consentStatus)
                query.consentStatus = clientQuery.consentStatus;
            if (clientQuery.classes)
                query.classes = clientQuery.classes;
            if (clientQuery.firstName)
                query.firstName = clientQuery.firstName;
            if (clientQuery.lastName)
                query.lastName = clientQuery.lastName;
    ...
    async patch(_id, _data, _params) {
        ...
        const users = await this.prepareRoleback(email, _id, () => this.app.service('usersModel').patch(null, _data, params));
        if (users.length === 0)
            throw new BadRequest('user could not be edit');
        return users[0];
    }
    /**
    ...
    async checkMail(email, userId) {
        if (email) {
            ...
            if (userId && user.total === 1 && equalIds(user.data[0]._id, userId))
                return;
            if (user.total !== 0) {
