...
const getProtectedRoles = (hook) => hook.app.service('/roles').find({
...
const removeStudentFromClasses = async (hook) => {
    ...
    if (userIds === undefined) {
        throw new BadRequest('Der Nutzer wurde gelöscht, konnte aber eventuell nicht aus allen Klassen/Kursen entfernt werden.');
    }
    try {
        const usersClasses = await classesService.find({ query: { userIds: { $in: userIds } } });
        await Promise.all(usersClasses.data.map((klass) => classesService.patch(klass._id, { $pull: { userIds: { $in: userIds } } })));
    }
    catch (err) {
        throw new Forbidden('Der Nutzer wurde gelöscht, konnte aber eventuell nicht aus allen Klassen/Kursen entfernt werden.', err);
    }
    return hook;
};
const removeStudentFromCourses = async (hook) => {
    ...
    try {
        ...
        await Promise.all(usersCourses.data.map((course) => hook.app.service('courseModel').patch(course._id, { $pull: { userIds: { $in: userIds } } })));
    }
    catch (err) {
        throw new Forbidden('Der Nutzer wurde gelöscht, konnte aber eventuell nicht aus allen Klassen/Kursen entfernt werden.', err);
    }
...
const enforceRoleHierarchyOnDelete = async (context) => {
    if (context.id)
        return enforceRoleHierarchyOnDeleteSingle(context);
...
const includeOnlySchoolRoles = async (context) => {
    if (context.params && context.params.query) {
        ...
        if (context.params.query.roles && context.params.query.roles.$in) {
            ...
            context.params.query.roles.$in = context.params.query.roles.$in.filter((r) => allowedRoles.some((a) => ObjectId.equal(r, a)));
        }
        else {
