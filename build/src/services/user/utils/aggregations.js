...
const stageSort = (aggregation, sort) => {
    ...
    for (const k in sort) {
        ...
        else if ({}.hasOwnProperty.call(sort, k))
            mSort[k] = Number(sort[k]);
    }
    aggregation.push({
        $sort: mSort,
    });
 ...
 */
const createMultiDocumentAggregation = ({ select, sort, limit = 25, skip = 0, consentStatus, classes, schoolYearId, searchQuery, searchFilterGate, ...match }) => {
