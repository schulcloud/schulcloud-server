...
const registerUser = function register(data, params, app) {
    ...
    return Promise.resolve()
        ...
        .then(() => populateUser(app, data).then((response) => {
        ...
        if ((user.roles || []).includes('student')) {
            ...
            if (user.birthday instanceof Date && Number.isNaN(user.birthday)) {
                return Promise.reject(new BadRequest('Fehler bei der Erkennung des ausgewählten Geburtstages. Bitte lade die Seite neu und starte erneut.'));
            }
            // wrong age?
            const age = getAge(user.birthday);
            if (data.parent_email && age >= CONSENT_WITHOUT_PARENTS_MIN_AGE_YEARS) {
                return Promise.reject(new BadRequest(`Schüleralter: ${age} Im Elternregistrierungs-Prozess darf der Schüler` +
            ...
            if (!data.parent_email &&
                ...
                return Promise.reject(new BadRequest(`Schüleralter: ${age} Im Schülerregistrierungs-Prozess darf der Schüler` +
