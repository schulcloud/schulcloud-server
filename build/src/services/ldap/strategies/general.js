...
class GeneralLDAPStrategy extends AbstractLDAPStrategy {
     ...
     */
    async getUsers() {
        const { userAttributeNameMapping, userPathAdditions, roleType, roleAttributeNameMapping, } = this.config.providerOptions;
    ...
    getClasses() {
        ...
        if (classPathAdditions !== '') {
            const options = {
                filter: filterForModifiedEntities(this.config.lastModifyTimestamp, `${classAttributeNameMapping.description}=*`),
                scope: 'sub',
                attributes: [
