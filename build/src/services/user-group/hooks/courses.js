...
const addWholeClassToCourse = (hook) => {
    ...
    if ((requestBody.classIds || []).length > 0) {
        ...
        return Promise.all(requestBody.classIds.map((classId) => ClassModel.findById(classId)
            .exec()
            .then((c) => c.userIds))).then(async (studentIds) => {
...
const deleteWholeClassFromCourse = (hook) => {
    ...
    return CourseModel.findById(courseId)
        ...
        if (removedClasses.length < 1)
            return hook;
        return Promise.all(removedClasses.map((classId) => ClassModel.findById(classId)
            .exec()
            .then((c) => (c || []).userIds))).then(async (studentIds) => {
            // flatten deep arrays and remove duplicates
            studentIds = _.uniqWith(_.flattenDeep(studentIds), (e1, e2) => JSON.stringify(e1) === JSON.stringify(e2));
            // remove class students from course DB and from hook body to not patch them back
            await CourseModel.update({ _id: course._id }, { $pull: { userIds: { $in: studentIds } } }, { multi: true }).exec();
...
const courseInviteHook = async (context) => {
    if (context.path === 'courses' && context.params.query && context.params.query.link) {
        ...
        if (dbLink)
            return restrictToCurrentSchool(context);
    }
    return restrictToUsersOwnCourses(context);
};
const patchPermissionHook = async (context) => {
    const query = context.params.query || {};
    const defaultPermissionHook = (ctx) => Promise.resolve(checkScopePermissions(['COURSE_EDIT'])(ctx)).then((_ctx) => restrictToUsersOwnCourses(_ctx));
    if (query.link) {
