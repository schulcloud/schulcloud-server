...
const createHomework = (homework, courseId, lessonId, userId, app, newTeacherId) => app
    ...
    async create(data, params) {
        ...
        await Promise.all(homeworks.map((homework) => {
            ...
            if (!homework.lessonId) {
                return createHomework(homework, res._id, undefined, equalIds(params.account.userId, homework.teacherId) ? params.account.userId : homework.teacherId, this.app, params.account.userId);
            }
            return false;
        })).catch((err) => {
...
//# sourceMappingURL=course-copy-service.js.map