...
function isTeacher(userId, homework) {
    ...
    if (!isTeacherCheck && !homework.private) {
        ...
        const isCourseSubstitution = (homework.courseId.substitutionIds || []).some((substitutionId) => equalIds(substitutionId, user));
    ...
    return submissionService
        ...
        data = data.map((e) => {
            ...
            if (!isTeacher(hook.params.account.userId, copy)) {
                ...
                const filteredSubmission = currentSubmissions.filter((submission) => equalIds(hook.params.account.userId, submission.studentId) ||
            ...
            if (!copy.private &&
                ...
                const NumberOfUsersWithSubmission = validSubmissions
                    .map((e) => e.courseGroupId ? (e.courseGroupId.userIds || []).length || 1 : (e.teamMembers || []).length || 1)
                    .reduce((a, b) => a + b, 0);
...
const hasPatchPermission = (hook) => {
    ...
    return homeworkService
        ...
        if (isStudent && onlyChangesArchived) {
            ...
            const removedStudents = homework.archived.filter((studentId) => !hook.data.archived.find((stId) => equalIds(studentId, stId)));
            const removesOnlySelf = removedStudents.length === 1 && equalIds(removedStudents[0], hook.params.account.userId);
            // allow the user to only add him/herself to the archived array (archive homework for this user)
            const addedStudents = hook.data.archived.filter((studentId) => !homework.archived.find((stId) => equalIds(studentId, stId)));
        ...
        .catch((err) => {
        throw new GeneralError({ message: "[500 INTERNAL ERROR] - can't reach homework service @isTeacher function" }, err);
    });
};
const hasCreatePermission = async (context) => {
    ...
    if (data.courseId) {
        ...
        const userInCourse = course.userIds.some((id) => equalIds(id, userId)) ||
