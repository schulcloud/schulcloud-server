...
const deleteUser = (app) => {
    app.service('users').on('removed', async (result) => {
        ...
        await Promise.all(promises)
            ...
            logger.info(`Remove user ${userId} from the teams ${teamIds},` + 'if team.userIds empty then removed the team too.');
        })
            .catch((err) => {
            const notRemovedFromTeams = teams.filter((team) => team.userIds.some((teamUser) => equalIds(teamUser.userId, userId)));
            logger.warning(`Can not remove user ${userId} from the teams ${notRemovedFromTeams},` +
                'if team.userIds empty then removed the team too.', err);
        });
    });
};
module.exports = (app) => {
