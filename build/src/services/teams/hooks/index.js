...
const { Forbidden, NotFound, BadRequest, Conflict, NotImplemented, MethodNotAllowed, NotAcceptable } = reqlib('src/errors');
...
const { populateUsersForEachUserIdinHookData, getTeamUsers, arrayRemoveAddDiffs, getTeam, updateMissingDataInHookForCreate, getSessionUser, ifSuperhero, isAcceptWay, } = require('./helpers');
const { isArray, isArrayWithElement, isObject, isString, hasKey, isDefined, isUndefined, 
// isNull,
// isObjectId,
// isObjectIdWithTryToCast,
throwErrorIfNotObjectId, bsonIdToString, isSameId, isFunction, } = require('./collection');
/**
 ...
 *   */
const teamMainHook = globalHooks.ifNotLocal((hook) => Promise.all([getSessionUser(hook), getTeam(hook), populateUsersForEachUserIdinHookData(hook)])
        ...
        hook.data.userIds.forEach((_) => {
            ...
            if (isDefined(teamUser)) {
                if (isDefined(_.role) && !isSameId(teamUser.role, _.role)) {
                    hasChangeRole = true;
                    if (isHigherOrEqualTeamrole(hook, _.role, highestChangedRole))
                        highestChangedRole = _.role;
...
const addCurrentUser = globalHooks.ifNotLocal((hook) => {
    if (hasKey(hook.result, 'userIds')) {
        ...
        if (isUndefined([user, user.role], 'OR')) {
            logger.warning('Can not execute addCurrentUser for unknown user. ' +
                'Or user execute a patch with the result that he has left the team.', { userId });
            return hook;
        }
 ...
 * @beforeHook
 */
const isAllowedToCreateTeams = (hook) => getSessionUser(hook).then((sessionUser) => hook.app
    .service('schools')
...
exports.after = {
    ...
    update: [],
    patch: [isUserIsEmpty, addCurrentUser, pushUserChangedEvent],
