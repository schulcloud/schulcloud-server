...
const findDocuments = async (app, userId, theme) => {
    ...
    if (userId) {
        ...
        if (school.documentBaseDirType === 'school')
            query = { schoolId: school._id };
        if (school.documentBaseDirType === 'schoolGroup')
            query = { schoolGroupId: school.schoolGroupId };
    }
    const result = await helpDocumentsModel.find(query).lean().exec();
    if (result.length < 1)
        throw new NotFound('could not find help documents for this user or theme.');
