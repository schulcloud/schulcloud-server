...
exports.isSuperHero = () => (context) => {
    ...
    return userService.find({ query: { _id: context.params.account.userId || '', $populate: 'roles' } }).then((user) => {
...
exports.hasRoleNoHook = (context, userId, roleName, account = false) => {
    ...
    if (account) {
        return accountService.get(userId).then((_account) => userService.find({ query: { _id: _account.userId || '', $populate: 'roles' } }).then((user) => {
            user.data[0].roles = Array.from(user.data[0].roles);
...
const resolveToId = (service, key, value) => {
    ...
    return service.find({ query }).then((results) => {
        ...
        if (!result)
            throw new TypeError(`No records found where ${key} is ${value}.`);
        return result._id;
    });
};
...
const getUser = (context) => context.app
    .service('users')
...
exports.mapPayload = (context) => {
    if (!isProductionMode) {
        logger.info('DEPRECATED: mapPayload hook should be used to ensure backwards compatibility only, ' +
    ...
    Object.defineProperty(context.params, 'payload', {
        get() {
            if (!isProductionMode) {
                logger.warning('reading params.payload is DEPRECATED, please use params.authentication.payload instead!' +
        ...
        set(v) {
            if (!isProductionMode) {
                logger.warning('writing params.payload is DEPRECATED, please use params.authentication.payload instead!' +
                    `path: ${context.path} method: ${context.method}`);
            }
...
exports.denyIfNotCurrentSchool = ({ errorMessage = 'Die angefragte Ressource gehört nicht zur eigenen Schule!' }) => (context) => getUser(context).then((user) => {
    if (testIfRoleNameExist(user, 'superhero')) {
        return context;
    }
...
// meant to be used as an after context
exports.denyIfNotCurrentSchoolOrEmpty = ({ errorMessage = 'Die angefragte Ressource gehört nicht zur eigenen Schule!', }) => (context) => {
    if (!(context.result || {}).schoolId) {
        return context;
    }
    return this.denyIfNotCurrentSchool(errorMessage)(context);
};
exports.denyIfStudentTeamCreationNotAllowed = ({ errorMessage = 'The current user is not allowed to list other users!', }) => async (context) => {
