...
const { NODE_ENV, DATABASE_AUDIT, MONGOOSE_CONNECTION_POOL_SIZE, DB_URL, DB_USERNAME, DB_PASSWORD, ENVIRONMENTS, } = require('../../config/globals');
...
const splitForSearchIndexes = (...searchTexts) => {
    ...
    searchTexts.forEach((item) => {
        item.split(' ').forEach((it) => {
            ...
            if (it.length > 1)
                arr.push(it.slice(0, 2));
            for (let i = 0; i < it.length - 2; i += 1)
                arr.push(it.slice(i, i + 3));
        });
    });
    return arr;
};
...
function connect() {
    ...
    logger.info('connect to database host', options.url, options.username ? `with username ${options.username}` : 'without user', options.password ? 'and' : 'and without', 'password');
