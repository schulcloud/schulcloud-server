...
const handleAutoLogout = async (context) => {
    const ignoreRoute = typeof context.path === 'string' && AUTO_LOGOUT_BLACKLIST.some((entry) => context.path.match(entry));
...
const leadTimeDetection = (context) => {
    if (context.params.leadTime) {
        ...
        if (timeDelta >= LEAD_TIME) {
            const { path, id, method, params: { query, headers, originalUrl }, } = context;
            const info = {
                path,
                method,
                query,
